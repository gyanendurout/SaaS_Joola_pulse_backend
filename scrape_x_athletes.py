"""
JOOLA X / Twitter Athlete Scraper via Apify REST API
=====================================================
Fetches recent tweets from JOOLA-sponsored athletes using
Apify's apidojo/tweet-scraper actor, then upserts into
the influencer_posts table in Supabase.

USAGE
-----
  python scrape_x_athletes.py               # fetch last 50 tweets per athlete
  python scrape_x_athletes.py --tweets 100  # fetch more
  python scrape_x_athletes.py --dry-run     # fetch & print, no DB write
...
ATHLETES = [
    {"influencer_id": "db4b18f4-da4b-49fd-a1e8-64b52ea8ae3b", "name": "Ben Johns",      "x_handle": "BenJohns_pb"},
    {"influencer_id": "9a925fc4-d080-4b40-a566-4de9be16f1b9", "name": "Anna Bright",    "x_handle": "AnnaBright108"},
    {"influencer_id": "1eb20d81-3dff-4fcb-ab0f-be93768af354", "name": "Tyson McGuffin", "x_handle": "TysonMcGuffin"},
    {"influencer_id": "d72173e1-ea3a-4baf-8f3e-820732b625ea", "name": "Bobbi Oshiro",   "x_handle": "BobbiOshiro"},
    {"influencer_id": "5ed1c771-d801-49de-b2bc-d2db62cae177", "name": "Jorja Johnson",  "x_handle": "JorjaJohnson"},
    {"influencer_id": "1989b854-0879-4e08-aff2-53345045fb80", "name": "Patrick Smith",  "x_handle": "PatrickSmithPB"},
]
...
# ── Scrape one athlete ────────────────────────────────────────────────────────
def scrape_athlete(athlete: dict, max_tweets: int) -> list[dict]:
    handle = athlete["x_handle"]
    name   = athlete["name"]
    ...
    for item in items:
        ...
        ts = _parse_ts(item.get("createdAt"))
        posted = datetime.fromisoformat(ts) if ts else None

        like_count    = _safe_int(item.get("likeCount"))
        retweet_count = _safe_int(item.get("retweetCount"))
        reply_count   = _safe_int(item.get("replyCount"))
        view_count    = _safe_int(item.get("viewCount"))

        author_handle = (item.get("author") or {}).get("userName") or handle
        post_url = item.get("url") or f"https://x.com/{author_handle}/status/{tweet_id}"
        text = (item.get("text") or item.get("fullText") or "")[:2000] or None
        ...
        print(f"  {tweet_id}  likes={like_count}  views={view_count}  rts={retweet_count}  {date_str}")
...
# ── Main ──────────────────────────────────────────────────────────────────────
def main() -> None:
    parser = argparse.ArgumentParser(description="Scrape JOOLA athlete tweets → Supabase via Apify")
    parser.add_argument("--tweets",  type=int, default=50,   help="Max tweets per athlete (default 50)")
    ...
    for athlete in ATHLETES:
        rows = scrape_athlete(athlete, args.tweets)
        all_rows.extend(rows)

    print(f"\n{'='*50}")
    print(f"Total: {len(all_rows)} tweets across {len(ATHLETES)} athletes")
    ...
    print("\nDone! Refresh the Influencers dashboard to see X/Twitter data.")
